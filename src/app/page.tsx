...
export default function Home() {
  ...
  const [showToast, setShowToast] = useState<null | { type: "success" | "error"; msg: string }>(null);
  const [errors, setErrors] = useState<{ firstName?: string; lastName?: string }>({});
  const [formTitle, setFormTitle] = useState("EventReg");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setForm({ firstName: "", lastName: "", email: "", phone: "", heardFrom: [] });
      } else {
    ...
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 flex flex-col items-center py-4 px-2 sm:px-6 transition-colors relative">
      ...
      <div className="mt-16 ">
        <header className="w-full flex justify-center items-center mb-8 px-2 sm:px-4">
          <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100">{ formTitle }</h1>
        </header>
        {/* Registration Form */ }
          ...
          >
            ...
            <div>
              <label className="block text-lg font-medium text-gray-700 dark:text-gray-200 my-6">
                How did you hear about this event? <span className="text-sm">(Select all that apply)</span>
              </label>
              <div className="mt-2 flex flex-wrap gap-x-4 gap-y-2">
                { ['Instagram', 'Facebook', 'Twitter', 'TikTok', 'LinkedIn', 'Friend', 'Other'].map((option) => (
